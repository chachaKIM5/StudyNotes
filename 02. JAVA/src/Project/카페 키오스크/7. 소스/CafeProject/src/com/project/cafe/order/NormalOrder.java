		...
		while (loop) {
			...
			if (MenuCategory.chosenCategory.equals("*")) { // 장바구니 선택 시 메뉴선택과정 메소드 실행되지 못하도록
				...
				// continue;
					...
					|| MenuCategory.chosenCategory.equals("3") || MenuCategory.chosenCategory.equals("4")
				...
				while (subsubLoop) {
					...
					if (Main.customerLogin(tel)) {
						...
						if (Data.cmlist.stream()
								.filter(cmMenu -> FindData
										.findCustomer(cmMenu.getClientSeq()).getTel().equals(tel))
								.count() < 5) {

							Data.cmlist.add(m);
							Data.save(DataPath.커스텀메뉴);
