...
public class Ex74_Lambda {
	...
	public static void main(String[] args) {
		 ...
		 *** 람다식을 왜 사용합니까? > 생산성
		     ...
		     > 익명 객체 특칭 포함(1회용 객체를 생성할 때)
		     
		     
		 함수형 인터페이스, Functional Interface
		  - 람다식을 사용하기 위해 JDK에서 제공하는 인터페이스
		  - 다른 용도로는 사용 X, 오직 람다식을 저장하는 용도로만 사용함
	...
	private static void m6() {
		...
		//Function<Integer,Integer> f5 = f3.andThen(f4); 아래와 같은 결과
	...
	private static void m5() {
		...
		System.out.println(p2.test("홍길동", "홍길동"));
		System.out.println(p2.test("홍길동", "홍아무개"));
		System.out.println(p3.test("홍길동", "홍아무개"));
	}

	private static void m4() {		//Operator
		
		//Operator
		// - 매개변수를 전달하면 처리 후 반환값을 돌려주는 업무를 구현
		// - applyXXX() 추상 메소드 제공
		// - 추상 메소드의 매개변수와 반환값의 자료형이 동일하다.
		...
		System.out.println(bo2.apply("홍길동", "입니다"));
		
		
	}

	private static void m3() {		//Function

		//Function
		// - 매개변수를 전달하면 처리 후 반환값을 돌려주는 업무 구현
		// - applyXXX() 추상 메소드 제공
		...
		System.out.println(f3.apply(new User("1", "홍길동", 25, "ckwkd", null)));
	...
	private static void m2() {		//Supplier
	...
	private static void m1() {		//Consumer
...
// > 이 1개의 추상메소드가 곧 람다식(익명 메소드)이 되기 때문에
@FunctionalInterface //1. 남에게 역할 알리기 위해 2. 문법적인 체크(추상 메소드 1개인지 검사)
...
interface Test {
	...
	public static void bbb() { //개인 행동보다는 모두의 행동
