...
select
    substr(email, instr(email, '@') + 1) as "Long Domain",
    round(avg(length(substr(email, 1, instr(email, '@')-1))), 5) as "avg"
from tblAddressBook
...
from tblAddressBook where instr(email, 'gmail.com') > 0
...
select
    *
from tblAddressBook
    where name = (select name from tblAddressBook group by name
                        having count(name) = (select max(count(name)) from tblAddressBook group by name));
...
select
    ...
    round(count(case
        when floor(age / 10) = 2 then 2
    end)/count(*)*100,1) || '%' as "[20대]",
    
    round(count(case
        when floor(age / 10) = 3 then 3 
    end)/count(*)*100,1) || '%' as "[30대]",
    
    round(count(case
        when floor(age / 10) = 4 then 4 
...
select
    ...
    s.address as "주소",
    s.salary as "월급",
    j.project as "담당 프로젝트명"
from tblStaff s
    left outer join tblProject j
        on s.seq = j.staff_seq;
...
select
    '20' || substr(e.hire_date, 1, 2) as "입사년도",
    avg(salary) as "평균 급여"
...
select e.employee_id as "사번", e.first_name || ' ' || e.last_name as "이름" from jobs j
    inner join job_history h 
...
select
    a.first_name,
    a.last_name,
    a.hire_date as "직원 고용일"
from employees a           --직원 테이블
    inner join employees b --매니저 테이블
        on a.manager_id = b.employee_id
