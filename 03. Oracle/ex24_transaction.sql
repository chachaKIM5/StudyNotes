...
commit;     -- 현재 트랜잭션에서 했던 모든 행동을 진짜 데이터베이스에 반영한다
rollback;   -- 현재 트랜잭션에서 했던 모든 행동을 진짜 데이터베이스에는 반영 X, 없었던 일로 취소(되돌린다)
...
update tblCountry set capital = '세종' where name = '대한민국'; --업데이트

select * from tblCountry;                                       --제대로 변경됐는지 확인?
...
/*
    ...
    현재 트랜잭션이 종료되는 경우
    1. commit 실행 > 현재 트랜잭션을 DB에 반영하고 종료
    2. rollback 실행 > 현재 트랜잭션을 DB에 반영 안 함
    3. 클라이언트 접속 종료
        a. 정상 종료
            - 현재 트랜잭션에 아직 반영 안 된 명령어가 남아 있으면 사용자에게 질문
        b. 비정상 종료
            - 무조건 rollback 처리
    4. DDL 실행
...
commit; -- 지금부터 새로운 트랜잭션 시작!

delete from tblTrans where buseo = '영업부'; -- 불안한 부분 있다면? rollback 위해서
...
rollback to b; -- 개발부가 되살아남

select * from tblTrans;

rollback to a; -- '김말숙'이 되살아남

select * from tblTrans;

rollback;      -- '가가가', '나나나' 사라짐
