...
select
    ...
    level
from tblComputer
    --start with seq = 2               -- 최상위 노드 지정, seq = 2(본체) > 본체 포함 하위 항목만 보겠다는 뜻, 일부 가지만 볼 수 있게
    --start with seq = (select seq from tblComputer where name = '모니터)
    start with pseq is null            -- 부모 seq가 없다 -> 최고 꼭대기부터 시작(=컴퓨터)
        connect by prior seq = pseq;   -- 레코드와 부모 레코드를 연결하는 조건, 셀프조인의 on 절과 똑같은 역할
