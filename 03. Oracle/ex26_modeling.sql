...
/*
    ...
    데이터 모델링
    - 요구 분석을 기반으로 수집한 데이터 > 분석 > 저장 구조 생성 > 도식화 > ERD
    - 데이터 저장하기 위한 데이터 구조를 설계하는 작업
    - DBMS의 종류를 결정하지 않는다. > 추상화
    - 개념 + 논리 + 물리
    - 개념 > 아주 간단하게 표현한 설계도 > 테이블 이름과 관계 정도만 기술
...
4. 관계 맺기 (*******) 부모를 먼저 누르고 자식 누르기

----------------- 논리 다이어그램, 논리 ERD ---------------------

5. 물리명(식별자) 생성
6. 자료형 지정, 길이 지정, 도메인 설정, 제약사항 생성
...
----------------- 물리 다이어그램, 물리 ERD ---------------------
    ...
    2. 갱신 이상, Update Anomaly
        - 동일한 데이터가 2개 이상의 테이블에 동시에 저장 > 그 중 1개는 수정, 다른 하나는 수정 못 한 경우
        - 두 데이터 간의 이상 오류 발생
        - 즉, 2개의 데이터 중 어떤 데이터가 올바른 데이터인지 구분이 불가능
        
    3. 삭제 이상, Deletion Anomaly
        - 특정 테이블의 데이터를 삭제살 때 원하지 않는 데이터까지 같이 지워지는 상황
    ...
    - 부분 함수 종속 발견!! > 부분 함수 종속 제거하는 과정이 제2정규화
    - 주로 복합키를 가지는 테이블에서 발견되며,
      일부 컬럼이 복합키 모두에게 종속이 아니라, 복합키 일부에만 종속되는 현상이 발견
        > 현상 제거
        
        ** 복합키: 두 컬럼을 합쳐서 기본키가 된 경우
        ** 완전 함수 종속: 기본키가 아닌 컬럼이 기본키에게 종속적인 상태
        
    제3정규화, 3NF
    - 기본 키가 아닌 다른 컬럼에게 종속되는 컬럼은 존재할 수 없다.
    - 이행 함수 종속 발견!! > 이행 함수 종속 제거하는 과정이 제3정규화
