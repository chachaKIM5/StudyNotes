...
/*
    ...
    - SQL 명령 처리 속도를 빠르게 하기 위해 특정 컬럼에 대해 생성되는 검색 도구
    - b-tree 구조
    ...
    인덱스의 장단점
    - 처리 속도를 향상시킨다.
    - 인덱스 사용은 전체적인 DB의 성능을 저하시킨다.
    
    자동으로 인덱스가 걸리는 컬럼
    1. Primary Key
    2. Unique
    - >> 일반 테이블에서 PK를 검색하는 속도와 아닌 컬럼을 검색하는 속도가 차이난다
    ...
    인덱스를 사용하지 말아야 하는 경우
    1. 테이블에 데이터(행)가 적은 경우 (의미가 없다)
    2. 인덱스의 손익분기점 > 검색 결과가  15% 이상인 경우
    3. 해당 테이블이 삽입, 수정, 삭제가 빈번한 경우 (**********************)
...
select count(*) from tblIndex; -- 7733248건 (약 770만 건)
...
-- 인덱스 없이 검색, F5 눌러 실행하면 경과 시간 확인 가능
-- 2.788초
...
--   - 색인의 값이 중복 불가능
--   - PK, UNIQUE
create unique index idxName on tblIndex(name); -- 동명이인이 없다고 가정 하에
...
select count(*) from tblIndex where name = '홍길동' and buseo = '기획부'; --버전에 따라 순서를 구분하기도 한다
...
--  5. 함수 기반 인덱스
create index idxSsn on tblIndex(ssn); --(X)
...
-- 결론: 더미 데이터 확보 + 업무 SQL 테스트 -> 평균 시간 -> index -> 검토 -> index -> 검토...
-- 프로젝트 마무리 > 포인터 2~3군데 잡아 index 걸기
-- 가장 많이 쓰는, 반복되는 where절 or 반복되는 join