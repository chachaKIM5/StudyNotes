...
public class Ex03_Statement {
	...
	private static void m4() {
		...
		try {
			...
			//***** 자바 자료형과 오라클 자료형과는 아무런 연관성 X
			//tblAddress의 age 컬럼에 number 자료형이 들어간다고 해도 자바의 age는 String여도 상관 X
	...
	private static void m3() {
		...
		try {
			...
			//String sql = "create sequence seqJava"; //결과 0이지만 정상 실행
			//String sql = "drop sequence seqJava";	  //결과 0이지만 정상 실행
	...
	private static void m1() {
		...
		try {
			...
			if (!conn.isClosed()) {
				...
				//2.
				//- 자바는 SQL을 모른다. SQL을 '아무 의미 없는' 문자열로 취급한다.
				...
				//반환값이 없는 쿼리 > 나머지, int값 = 적용된 행의 개수
				//int stat.executeUpdate(sql)
				
				//반환값이 있는 쿼리 > select
				//ResultSet stat.executeQuery(sql)
				
				//SQL Developer > Ctrl + Enter와 동일 
				int result = stat.executeUpdate(sql);
