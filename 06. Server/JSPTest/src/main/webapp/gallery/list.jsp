...
<body>
	...
	<div class="container">
		...
		<div id="list">
			...
			<% while (rs.next()) { %>
				...
				<span title="delete" onclick="deleteImage('<%= rs.getString("seq") %>', '<%= rs.getString("filename") %>');">&times;</span>
	...
	<script>
		...
		function deleteImage(seq, filename) {
			//alert(img);
						
			if (confirm('delete?')) { 
				location.href = 'delok.jsp?seq=' + seq + '&filename=' + filename;
			}
