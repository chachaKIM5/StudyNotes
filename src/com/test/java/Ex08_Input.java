...
public class Ex08_Input {
	...
	public static void main(String[] args) throws Exception { //예외 처리
		...
		//BufferedReader는 한 번 만들어 놓으면 계속 사용 가능
		...
		//일시 중지 > "블럭 걸렸다" 표현함 > 사용자가 키보드로 무언가를 입력하기를 기다리고 있는 중..
		//사용자가 입력한 데이터를 이 위치에 남기고 사라짐
//		System.out.print("단어: ");
//		
		...
		/*
		 ...
		 에러, Error
		  ...
		  2. 런타임 에러
		  	...
		  	- 문법에는 오류 X, 다른 원인으로 발생하는 에러
		  	- 예외(Exception)
		  	- 난이도 중간 > 발견 중간 > 발견 복불복 >  테스트 > 	발견O > 코드 수정
		  											  	발견X > 조치 X
		  	- 게임 출시 > RPG > 소수의 개발자가 모든 기능 테스트 > 모든 기능 테스트 불가능
		  	 		  > 클로즈드 알파, 클로즈드 베타 테스트 > 오픈 알파, 오픈 베타 테스트 > 수정..
		  	- 지구상의 모든 프로그램은 런타임 오류를 내재
		  	- 패치, 업그레이드..
		  	- 해결 방법: 다양한 테스트 > 발견된 오류에 한해서 수정
		  	
		  3. 논리 에러
		 	- 컴파일 + 실행 성공 > 결과 중 일부 이상
		 	- 난이도 최상 > 발견 최악 + 수정 최악
		  ...
		  ** 오류 노트 만들기
		  ...
		  - 에러 메시지 	> 런타임 에러의 경우 메시지의 첫 줄을 복사
		  				> java.lang.ArithmeticException: / by zero
		  - 에러 발생 코드 > System.out.printf("100 / %d = %d\n", num, 100 / num);
		  - 해결책 		> 제수를 0을 사용하면 안 된다.
